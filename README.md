...
To use this plug-in simply add `[test2junit "1.2.7"]` either to your `~/.lein/profile.clj` or as
...
Below is an example for globally adding test2junit via the ~/.lein/profiles.clj    file:
    ...
    {:user {:plugins [[test2junit "1.2.7"]]}}

Or you can just add it to your development profile like so:

    :profiles {:dev {:plugins [[test2junit "1.2.7"]]}}
